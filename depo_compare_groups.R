depo_compare_groups = function(simmr_out,source_name=simmr_out$input$source_names[1],groups=1:max(depo$Code),plot=TRUE) {
  ...
  if(length(groups)>2) {
    ...
    if(plot) {
      ...
      p = ggplot(df,aes(x=Group, y=Proportion))+
        ...
        xlab(expression("Deposit Feeders"))+
        ylab(paste("Proportion of ", source_name))+
        theme_bw()+ 
        coord_flip()+
        theme(legend.position='none',
        ...
        scale_x_discrete(limits=rev(levels(df$Group)),
                         labels=rev(levels(depo$Species)))+
