susp_compare_groups = function(simmr_out,source_name=simmr_out$input$source_names[1],groups=1:8,plot=TRUE) {
  ...
  if(length(groups)>2) {
    ...
    if(plot) {
      ...
      p = ggplot(df,aes(x=Group, y=Proportion)) + 
        ...
        xlab(expression("Suspension Feeders")) +
        ...
        scale_x_discrete(limits=rev(levels(df$Group)),
                         labels=rev(levels(susp$Species))
                         )+
        scale_y_continuous(limits=c(0,1))+ 
        coord_flip()
        #annotate("text", x=9, y=1, label="c", size=7)
